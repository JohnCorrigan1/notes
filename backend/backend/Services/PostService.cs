...
public class PostService : IPostService
{
    ...
    {
        return await _context.Connection.QueryAsync<Post>("SELECT * FROM posts ORDER BY PostedDate DESC");
    }

    public async Task<Post> GetPostByPostId(int id)
    {
        var sql = "SELECT * FROM posts WHERE id = @id";
        var posts = await _context.Connection.QueryFirstOrDefaultAsync<Post>(sql, new { id = id }) ?? throw new UserNotFoundException("Post not found");
        return posts;
    }
    ...
    {
        var sql = "INSERT INTO posts (post_id, title, body, posted_date, likes, author_id) VALUES (@post_id, @title, @body, @posted_date, @likes, @author_id) RETURNING id";
        await _context.Connection.ExecuteAsync(sql, new { post_id = post.PostId, title = post.Title, body = post.Body, posted_date = post.PostedDate, likes = post.Likes, author_id = post.AuthorId });
    }

    public async Task UpdatePost(Post post)
    {
        var sql = "UPDATE posts SET title = @title, body = @body, posted_date = @posted_date, likes = @likes, author_id = @author_id";
        var id = await _context.Connection.ExecuteAsync(sql, new { title = post.Title, body = post.Body, posted_date = post.PostedDate, likes = post.Likes, author_id = post.AuthorId });
        if (id == 0)
        {
    ...
    {
        var sql = "DELETE FROM posts WHERE id = @id RETURNING id";
        var post = await _context.Connection.ExecuteAsync(sql, new { id = id });
        if (post == 0)
        {
            throw new UserNotFoundException("Post not found");
        }
    ...
    public async Task CreateComment(Comment comment)
    {
        ...
        await _context.Connection.ExecuteAsync(sql, new { id = comment.Id, body = comment.Body, posted_date = comment.PostedDate, likes = comment.Likes, user_id = comment.UserId, post_id = comment.PostId });
    }
